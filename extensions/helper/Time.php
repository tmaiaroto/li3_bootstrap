...
class Time extends \lithium\template\Helper {
	...
	public function is($question, $date = null, array $options = array()) {
		$date = ($date instanceOf MongoDate) ? $date->sec:$date;
		switch ($question) {
			case 'leap year' :
	...
	protected function _words($date, array $options = array()) {
		$defaults = array(
			'offset' => 0, 'format' => 'j/n/y', 'end' => '+1 month', 'detail' => array('y', 'm', 'w', 'd', 'h', 'i', 's'), 'now' => date('Y-m-d H:i:s')
		);
		$options += $defaults;
		...
		foreach ($strings as $key => $text) {
			...
			$full_result[] = $value . ' ' . $text[($value == 1) ? 0 : 1];
			
			if(in_array($key, $options['detail'])) {
				$result[] = $value . ' ' . $text[($value == 1) ? 0 : 1];
			}
		}
		
		$time_in_words = join(', ', $result);
		if(empty($time_in_words)) {
			// if the detail level was too large for the time, show the full result.
			// make sure not to specify a detail level too short. it would be inaccurate then.
			// for example, passing array('s') ... would only show the seconds ONLY
			// so 2 days and 30 seconds ago would only show 30 seconds ago and it would be wrong.
			// passing array('y', 'm', 'd') is pretty safe. that will simply cut off the minutes
			// and seconds.
			$time_in_words = join(', ', $full_result);
		}
		return $time_in_words . ($date < $now ? ' ago' : '');
	}
